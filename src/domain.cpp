...
Array4d Domain::create_NS_array(const Params& p, const Decomposition& dec) const {
    ...
    size_t nx  = node_count[0];
    size_t ny  = node_count[1];
    size_t nz  = node_count[2];
    size_t ngc = p.get_number_of_ghostcells();
    // size_t nq = p.dim + 1 + 1; //this is the number of conservative and primitive variables for
    // N-S
    size_t nq = 5; // for now
...
void Domain::create_physical_west_boundary(const Params& p) {
    ...
    if (search->second == "Neumann") { m_bc_west = new Neumann(search->second, 0, m_conservative); }
...
void Domain::create_physical_east_boundary(const Params& p) {
    ...
    if (search->second == "Neumann") { m_bc_east = new Neumann(search->second, 1, m_conservative); }
}
...
void Domain::create_physical_back_boundary(const Params& p) {
    ...
    if (search->second == "Neumann") { m_bc_back = new Neumann(search->second, 5, m_conservative); }
}
