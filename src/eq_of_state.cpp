...
void EqOfState::primitive_to_conservative(const Array4d& W, Array4d& U){
	...
	for (size_t k = W.K_BEGIN(); k < W.K_END(); k++){	
		...
		//0.5 * (u*u + v*v + w*w)
		double kin =  0.5 * W(i,j,k,0) * ( W(i,j,k,2) * W(i,j,k,2) + W(i,j,k,3) * W(i,j,k,3) + W(i,j,k,4) * W(i,j,k,4) ); 
...
void EqOfState::conservative_to_primitive(const Array4d& U, Array4d& W){
	...
	for (size_t k = W.K_BEGIN(); k < W.K_END(); k++){	
		...
		//0.5 * (u*u + v*v + w*w)
		double kin =  0.5 * (W(i,j,k,2) * W(i,j,k,2) + W(i,j,k,3) * W(i,j,k,3) + W(i,j,k,4) * W(i,j,k,4) ); 
...
double EqOfState::local_dt(double rho, double p, double u, double v, double w, double dx, double dy, double dz){
