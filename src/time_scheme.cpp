...
void TimeScheme::step(Domain& domain, const Grid& grid, const Communicator& communicator, ConvectionScheme* convection, DiffusionScheme* diffusion){
	...
	*/
	...
	mult1 = 1.0; mult2 = 0.0; mult3 = -m_current_dt;
	...
	mult1 = 3.0/4.0; mult2 = 1.0/4.0; mult3 = -(1./4.)*m_current_dt;
	convection->solve(grid, W, U, R);
	diffusion->solve(grid, communicator, W, R);
	substep(U, Uprev, R, mult1, mult2, mult3);
	...
	communicator.communicate(U); 
	m_eq_of_state.conservative_to_primitive(U, W); //to update W halos
	mult1 = 1./3.; mult2 = 2./3.; mult3 = -(2./3.)*m_current_dt;
	convection->solve(grid, W, U, R);
	diffusion->solve(grid, communicator, W, R);
	substep(U, Uprev, R, mult1, mult2, mult3);
...
void TimeScheme::substep(Array4d& now, const Array4d& prev, const Array4d& increment, double mult1, double mult2, double mult3){
	...
	for (size_t q = now.q_begin(); q < now.q_end(); q++){	
		...
		now(i,j,k,q) = (prev(i,j,k,q)*mult1) + (now(i,j,k,q)*mult2) + (increment(i,j,k,q)*mult3);
