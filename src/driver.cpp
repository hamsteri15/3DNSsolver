...
void Driver::run(){
	...
	OutputInfo output_info = OutputInfo(parameters.io_interval, "3d fields", "results/primitive_fields_3d.h5");
...
void Driver::set_initial_condition(Domain& domain, const Grid& grid, const Params& p, const Communicator& comm){
	
	
	
	if (p.testcase == "sod_x"){ testcases::sod_xdir(domain, grid, p);}
	
	else if (p.testcase == "sod_y"){ testcases::sod_ydir(domain, grid, p);}
	else if (p.testcase == "sod_z"){ testcases::sod_zdir(domain, grid, p);}
	else if (p.testcase == "case3"){ testcases::case3(domain, grid, p);}
	else if (p.testcase == "case4"){ testcases::case4(domain, grid, p);}
	else if (p.testcase == "case6"){ testcases::case6(domain, grid, p);}
	else if (p.testcase == "case12"){ testcases::case12(domain, grid, p);}
	else if (p.testcase == "case15"){ testcases::case15(domain, grid, p);}
	else if (p.testcase == "case17"){ testcases::case17(domain, grid, p);}
	else if (p.testcase == "shear_xy"){ testcases::shear_layer_xy(domain, grid, p);}
	else if (p.testcase == "shear_3d"){ testcases::shear_layer_3d(domain, grid, p);}
