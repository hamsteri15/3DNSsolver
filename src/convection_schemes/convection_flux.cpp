...
void ConvectionFlux::euler_flux_from_primitive(const double* W, double* F, double gamma, double normal_x, double normal_y, double normal_z){
...
Array4d ConvectionFlux::form_flux(const Array4d& primitive, double gamma, double normal_x, double normal_y, double normal_z) const{
...
std::vector<double> ConvectionFlux::orthogonal_unit_vectors(std::vector<double> unit_normal) const{
	...
	double nx = unit_normal[0]; double ny = unit_normal[1]; double nz = unit_normal[2];
...
void ConvectionFlux::eigenvectors(double rho, double p, double u, double v, double w, double gamma, 
						  std::vector<double> n, std::vector<double> l, std::vector<double> m,
						  double* L, double* R ) const
{
	double nx = n[0]; double ny = n[1]; double nz = n[2];
	...
	double LL[5][5] =
		  { 
			{  0.5*K_pc*q2 + qn/(2.0*c), -(K_pc*u + nx/(2.0*c)), -(K_pc*v + ny/(2.0*c)), -(K_pc*w + nz/(2.0*c)), K_pc},
		    {  -qm,                      mx,                     my,                     mz,                     0.0 },
...
void ConvectionFlux::eigenvalues(const double* W, double* eig, double gamma, double nx, double ny, double nz){
...
double ConvectionFlux::max_eigenvalue(const double* W, double gamma, double nx, double ny, double nz){
...
Array3d ConvectionFlux::max_eigenvalue(const Array4d& primitive, double gamma, double normal_x, double normal_y, double normal_z) const{
