...
public:
    ...
    ///		\param (output)			F flux (rho*q_n, rho*q_n*H, rho*q_n*u + p*nx, rho*q_n*v + p*ny, rho*q_n*w
    ///+ p*nz)
    ///     \param (input)          gamma ratio of specific heats
    ///     \param (input)          normal_x, normal_y, normal_z normal vectors in cartesian
    ///     directions
    ////////////////////////////////////////////////////////////
    void euler_flux_from_primitive(const double* W,
                                   double*       F,
                                   double        gamma,
                                   double        normal_x,
                                   double        normal_y,
    ...
    ////////////////////////////////////////////////////////////
    ///
    ///    	\brief		Computes the left (L) and right (R) eigenvector matrices for 3D-Euler
    ///    system.
    ///					The syntax here corresponds to eq. 3.6.11 and 3.6.12 in IdoLikeCFD. Note the
    ///row 					shift in L and column shift in R to match the flux vector reordering.
    ///
    ///
    ///		\param (input)	W vector of primitive variables (rho, p,     u,     v,	  w) typically an
    ///avarage at W_{i + 1/2}
    ///     \param (input)          gamma ratio of specific heats
    ...
    ////////////////////////////////////////////////////////////
    ///
    ///    	\brief		Computes the eigenvalues eig = [lam1, lam2, lam3, lam4, lam5] for the 3D-Euler
    ///    system
    ///
    ///
    ///		\param (input)					W vector of primitive variables (rho, p,     u,     v,
    ///w) 		\param (output)					eig eigenvalues
    ///     \param (input)          		gamma ratio of specific heats
    ...
    Array4d form_flux(const Array4d& primitive,
                      double         gamma,
                      double         normal_x,
                      double         normal_y,
                      double         normal_z) const;

    Array3d max_eigenvalue(const Array4d& primitive,
                           double         gamma,
                           double         normal_x,
                           double         normal_y,
                           double         normal_z) const;
