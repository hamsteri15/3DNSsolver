...
Decomposition::Decomposition(const Params& p){
    ...
    m_num_domains[0] = p.nproc_x; m_num_domains[1] = p.nproc_y; m_num_domains[2] = p.nproc_z;
...
std::vector<int> Decomposition::get_subdomain_node_count(const Params& p) const{
    ...
    int block_i = get_subdomain_i(); int block_j = get_subdomain_j(); int block_k = get_subdomain_k();

    int num_domains_i = p.nproc_x; int num_domains_j = p.nproc_y; int num_domains_k = p.nproc_z;
...
std::vector<double> Decomposition::get_subdomain_start_coord(const Params& p) const{
    ...
    int start_i = start_idx[0]; int start_j = start_idx[1]; int start_k = start_idx[2];

    double dx = p.Lx / p.nx; double dy = p.Ly / p.ny; double dz = p.Lz / p.nz;
...
void Decomposition::cartesian_decomposition(const Params& p){
    ...
    int periodicity_info[3] = { z_periodicity_check(p),
								y_periodicity_check(p),
								x_periodicity_check(p)
							  };
