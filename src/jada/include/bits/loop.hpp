...
static constexpr auto md_indices(auto begin, auto end) {
    ...
    (std::make_integer_sequence<Idx, Idx(jada::rank(begin))>{});
}
...
static constexpr auto jada::all_indices(auto span) {
    using Idx = typename decltype(span)::index_type;
    return md_indices(std::array<Idx, jada::rank(span)>{}, dimensions(span));
}
