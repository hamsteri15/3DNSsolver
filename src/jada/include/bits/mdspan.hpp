...
using span_base = stdex::mdspan<ElementType, jada::extents<N>, Layout>;
...
template <class Span> static constexpr auto dimensions(const Span& span) {
    return jada::extent_to_array(extent(span));
}
...
template <class Container> static constexpr auto jada::make_span(Container& c, auto dims) {
    using value_type = typename Container::value_type;
    auto ext         = make_extent(dims);
    runtime_assert(flat_size(dims) == std::size(c), "Dimension mismatch in jada::make_span");
    return span<value_type, jada::rank(ext)>(std::data(c), ext);
}
...
/// @param dims dimensions of the multi-dimensional span
/// @return a multi-dimensional span
template <class Container> static constexpr auto jada::make_span(const Container& c, auto dims) {
...
template <class Span> void print(Span span) {
    ...
    if constexpr (jada::rank(span) == 1) {

        for (size_t i = 0; i < span.extent(0); ++i) { std::cout << span(i) << " "; }
        std::cout << std::endl;
    } else if constexpr (jada::rank(span) == 2) {
