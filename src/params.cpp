...
void Params::check_convection_scheme_validity() const{
	...
	if(item_in_vector(this->convection_scheme, valid_convection_schemes)) {
		...
		if(item_in_vector(this->convection_scheme_order, valid_convection_orders)) {
			...
			std::cout << "Convection scheme order: " << this->convection_scheme_order << " is not valid. Exiting." << std::endl;
			std::exit(1);
		}

		
		std::cout << " Convection in characteristic space? yes/no (0/1): " << this->convection_in_characteristic_space << std::endl;
	}

	else {
		std::cout << "Convection scheme: " << this->convection_scheme << " is not valid. Exiting." << std::endl;
		std::exit(1);
	}
...
void Params::check_testcase_validity() const{
	...
	std::vector<std::string> valid_test_cases = {"random", "sod_x", "sod_y", "sod_z", "case3", "case4",
												 "case6", "case12", "case15", "case17", "shear_xy", "jet", "rank", "shear_3d"};
...
void Params::check_diffusion_scheme_validity() const{
	...
	if(item_in_vector(this->diffusion_scheme, valid_diffusion_schemes)) {
		...
		std::cout << "Diffusion scheme: " << this->diffusion_scheme << " is not valid. Exiting." << std::endl;
		std::exit(1);
	}
	...
	if(item_in_vector(this->diffusion_scheme_order, valid_diffusion_orders)) {
		...
		std::cout << "Diffusion scheme order: " << this->diffusion_scheme_order << " is not valid. Exiting." << std::endl;
		std::exit(1);
	}
...
void Params::check_writer_validity() const{
	...
	if(item_in_vector(this->output_file_format, valid_output_formats)) {
		...
		std::cout << "Output file format: " << this->output_file_format << " is not valid. Exiting." << std::endl;
		std::exit(1);
	}
...
template<typename T>
int Params::item_in_vector(T item, std::vector<T> v) const {
