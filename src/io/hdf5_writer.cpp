...
Hdf5Writer::Hdf5Writer(const char* outfile_path)
    : Writer(outfile_path) {}

void Hdf5Writer::write(OutputInfo&          info,
                       const Grid&          grid,
                       const Domain&        domain,
    ...
    if (info.is_write_time()) {
        ...
        write_time_info_to_group(
            info.get_file_path(), info.get_current_checkpoint(), info.get_runtime());

        std::vector<std::string> datasets = {"rho", "p", "u", "v", "w"};

        write_4d_array(domain.get_NS_primitive_array(),
                       info.get_file_path(),
                       info.get_current_checkpoint(),
                       datasets,
                       p,
                       dec,
                       comm);
...
void Hdf5Writer::write_time_info_to_group(std::string f_path,
    ...
    if (world_rank == 0) {
        ...
        hid_t dataset = H5Dcreate(file_id,
                                  ...
                                  H5P_DEFAULT);
        hid_t err     = H5Dwrite(
            dataset, H5T_NATIVE_DOUBLE, memory_dataspace, file_dataspace, H5P_DEFAULT, &(runtime));
...
void Hdf5Writer::write_4d_array(const Array4d&           array,
...
void Hdf5Writer::create_and_write_dataset(double*     data,
                                          hid_t       file_id,
    ...
    hid_t dataset = H5Dcreate(file_id,
                              ...
                              H5P_DEFAULT);
    hid_t err     = H5Dwrite(
        dataset, H5T_NATIVE_DOUBLE, memory_dataspace, file_dataspace, plist_id, &(data[0]));
...
void Hdf5Writer::write_mesh(Domain* d) const {
    ...
    /*
        ...
        The datatype, dataspace and dataset objects should be released once they are no longer
    needed by a program. Since each is an independent object, the must be released (or closed)
    separately. The following lines of code close the datatype, dataspace, and datasets that were
