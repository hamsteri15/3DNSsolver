...
void CenteredDiffusion::solve(const Grid& grid, const Communicator& communicator, const Array4d& primitive, Array4d& residual ){
	...
	*/
	...
	compute_residual(primitive, dW_dx, dW_dy, dW_dz, 
					 ddW_dxdx, ddW_dydy, ddW_dzdz, 
					 dW_dxdy, dW_dxdz, 
					 dW_dydx, dW_dydz, 
					 dW_dzdx, dW_dzdy,
					 residual);
...
void CenteredDiffusion::compute_residual(const Array4d& primitive, const Array4d& Dx, const Array4d& Dy, const Array4d& Dz, 
						      const Array4d& Dxx, const Array4d& Dyy, const Array4d& Dzz,
							  const Array4d& Dxy, const Array4d& Dxz, 
							  const Array4d& Dyx, const Array4d& Dyz,
							  const Array4d& Dzx, const Array4d& Dzy,	
							  Array4d& residual)
{
	...
	for (size_t k = residual.k_begin(); k < residual.k_end(); k++){
		...
		double Dtauxx_dx = dtauxx_dx(mu, dmu_dx, du_dx, dv_dy, du_dxdx, dv_dydx, lamb, dlamb_dx);
		double Dtauyy_dy = dtauyy_dy(mu, dmu_dy, dv_dy, dv_dydy, du_dx, du_dxdy, lamb, dlamb_dy);
		...
		double Dtauvx_dx = U * Dtauxx_dx + V * Dtauxy_dx + Tauxx * du_dx + Tauxy * dv_dy;
        double Dtauvy_dy = U * Dtauyx_dy + V * Dtauyy_dy + Tauyy * dv_dy + Tauyx * du_dy;
        ...
        double temp2 =  mu * (2.0 * one_rho2 * drho_dx * dp_dx + one_rho * dp_dxdx + one_rho3 * P * drho_dx * drho_dx - one_rho2 * P * drho_dxdx);
        double temp3 = m_gamma / (m_Pr * (m_gamma - 1.0));
        double Dqx_dx = -temp3 * (temp1 + temp2);

		temp1 = dmu_dy * (one_rho * dp_dy - P * one_rho2 * drho_dy);
        temp2 = mu * (2.0 * one_rho2 * drho_dy * dp_dy + one_rho * dp_dydy + one_rho3 * p * drho_dy * drho_dy - one_rho2 * P * drho_dydy);
