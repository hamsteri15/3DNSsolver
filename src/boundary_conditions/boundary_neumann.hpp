...
public:
    ...
    Neumann(std::string type, int location, const Array4dDimensions& ad)
        : Boundary(type, location, ad) {
    ...
    Neumann(std::string type, int location, const Array4d& arr)
        : Boundary(type, location, arr) {
        m_first_domain_west  = m_ngc;
        m_first_domain_east  = m_gc_start_east - 1;
        m_first_domain_north = m_ngc;
        m_first_domain_south = m_gc_start_south - 1;
